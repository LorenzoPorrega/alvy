<li class="dark" x-data="{ open: false }" wire:click="selectRequestsList({{ $requestsList->id }}){{-- ,expandedRequestsList({{ $requestsList->id }}) --}}">
  <button @click="open = !open; if (open) { $wire.loadRequests() }"
    class="w-full flex items-center justify-between text-sm font-medium text-gray-800 dark:text-gray-200
          ...
          hover:bg-slate-300 hover:dark:bg-slate-600 focus-within:border-l-2 focus-within:border-honey-500
          border-l-2 border-l-transparent transition-colors duration-300 ease-in-out">
