<li wire:click="selectRequest({{ $request->id }})" class="relative">
  <div
    ...
    <button type="button" id="menu-button" aria-expanded="true" aria-haspopup="true"
      x-on:click="open_{{ $request->id }} = !open_{{ $request->id }}" {{-- wire:click="toggleDropdown({{ $request->id }})" --}}
      class="flex mr-2 h-6 px-[6px] hidden group-hover/request:block hover:bg-slate-200 hover:dark:bg-slate-500 rounded-[1.75px] focus:outline-none">
    ...
    <!-- Dropdown Menu -->
    <div x-show='open_{{ $request->id }}' {{-- wire:click.away="$set('activeDropdownId', null)" --}}
      ...
      role="menu" aria-orientation="vertical" aria-labelledby="menu-button" tabindex="-1" {{-- style="display: {{ $activeDropdownId === $request->id ? 'block' : 'none' }};" --}}>
