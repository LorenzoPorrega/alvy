    ...
    <div class="h-full flex flex-col">
      <div class="flex overflow-x-scroll overscroll-x-none no-scrollbar">
        @foreach ($openedTabs as $tabId => $tab)
          <div
            class="relative overflow-hidden max-w-40 border-r border-solid {{ $activeTab === $tabId ? 'border-b border-b-honey-500' : 'border-b-0' }} border-r-gray-50 dark:border-r-gray-700 group/tab">
            <button wire:click.stop="selectTab({{ $tabId }})"
              class="px-4 py-2 h-full max-w-40 flex text-gray-800 dark:text-gray-200 hover:bg-slate-300 hover:dark:bg-slate-600 
              {{ $activeTab === $tabId ? 'mb-1 bg-gray-50 dark:bg-gray-800' : '' }}">
              @if ($method)
                {{ $method->type }}
              @else
              ...
              <div
                class="h-full absolute z-50 top-0 right-0 min-w-8 flex items-center justify-center group-hover/tab:bg-slate-300  
      ...
      @foreach ($openedTabs as $tabId => $tab)
        @if ($tab['type'] === 'requestsList' && $activeTab === $tabId)
          <livewire:components.detailed.request-list :tabId="$tabId" :requestsListId="$tab['id']" :key="$tabId" />
        @elseif ($tab['type'] === 'request' && $activeTab === $tabId)
          <livewire:components.detailed.request :tabId="$tabId" :requestId="$tab['id']" :key="$tabId" />
        @endif
      @endforeach
    </div>
  @endif
